...
statistical basis outstanding` (the offset row: its deterministic
systematic term is ratified, while the mismatch/Monte Carlo basis it
owes is not — tracked by #17); `Ratified, systematic only [DR-2] —
mismatch-inclusive 3σ measured [P] (not yet re-ratified)` (the CMRR row:
its mismatch-inclusive evidence now exists — `sim/cmrr-mismatch/`,
issue #26 — but the DR-0002 residual-(c) re-ratification pass that would
supersede the systematic bound has not run); and `not ratifiable yet —
